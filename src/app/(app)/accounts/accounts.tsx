...
async function compileAddressesConcurrently(
  ...
  const tronBalancePromise= tronAcc.map(async (account: AddressState) => {
...
const AccountsIndex = () => {
  ...
  const createNewWalletPair = useCallback(async () => {
    ...
    try {
      ...
      const newTronWallet = await tronService.createWalletByIndex(
        phrase,
        nextTronIndex
      );
      ...
      const transformedTronWallet: AddressState = {
        accountName: `Account ${nextTronIndex + 1}`,
        derivationPath: newTronWallet.derivationPath,
        address: newTronWallet.address,
        publicKey: newTronWallet.publicKey,
        balance: 0,
        transactionMetadata: {
          paginationKey: undefined,
          transactions: [],
